...
public class RiskTrackerLogOut {
	...
	public RiskTrackerLogOut(RemoteWebDriver driver, String strConfig)
			throws InterruptedException {

		CommonLib.getLogger(strConfig).info("Inside RiskTrackerLogOut");
...
//		int iCurrentIteration = 1;
		...
		for (int j = 1; j <= iTotalIterations; j++) {
			for (int i = 0; i < businessrules.size(); i++) {
				...
				if (Value != "") {
					switch (Header) {
					case "BR_Logout_RiskTracker":
						BusinessRules.logoutRiskTracker(driver, strConfig);
						break;
