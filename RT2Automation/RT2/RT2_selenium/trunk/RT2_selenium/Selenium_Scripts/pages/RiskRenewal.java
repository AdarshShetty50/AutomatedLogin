...
public class RiskRenewal {
	...
	public RiskRenewal(RemoteWebDriver driver, String strConfig, String strTestingType, String strUserRole)
			throws InterruptedException {

		CommonLib.getLogger(strConfig).info("Inside RiskRenewal");
...
//		int iCurrentIteration = 1;
		...
		for (int j = 1; j <= iTotalIterations; j++) {
			for (int i = 0; i < businessrules.size(); i++) {
				...
				ArrayList<String> riskRenewalData = new ArrayList<String>();
				XSSFSheet riskRenewalSheet = read.readXLSXfile(ProjectConstants.RTTESTDATAEXCEL, ProjectConstants.RISKRENEWALSHEET);
				int rowCountRenew = riskRenewalSheet.getLastRowNum();
				int colCountRenew = read.columncount(riskRenewalSheet, 0);
				
				
				if (Value != "") {
					switch (Header) {

					case "BR_SearchRisk_And_Renew":
						...
						if (rowCountRenew > 2) {
							for (int k = 3; k <= rowCountRenew; k++) {
								for (int l = 0; l < colCountRenew; l++) {

									riskRenewalData.add(l, (read.CellValue(riskRenewalSheet, k, l)));
									System.out.println(riskRenewalData.get(l));

								}

								int nullCounter = 0;
								if (!riskRenewalData.isEmpty()) {
									for (int m = 0; m < riskRenewalData.size(); m++) {
										if (riskRenewalData.get(m) == null) {
											nullCounter = nullCounter + 1;
										}
									}

									if (!(nullCounter == riskRenewalData.size())) {
										Boolean riskFound = false;

										riskFound = BusinessRules.searchRisk_ByView(driver, strConfig,
												riskRenewalData.get(0), riskRenewalData.get(1), riskRenewalData.get(2),
												true);

										if (riskFound) {
											BusinessRules.renewRisk(driver, strConfig, riskRenewalData);
											BusinessRules.navigateToHomePage(driver, strConfig);	
										}
										riskRenewalData.clear();
							...
							CommonLib.getLogger(strConfig)
									.info("WARNING: No Risk Data supplied for Risk Renewal in RTTestdata.xlsx file");
							TestReporter.Warning(driver, strConfig, "Risk Renewal",
									"No Risk Data supplied for Risk Renewal in RTTestdata.xlsx file");
						}
						break;
