...
public class DMS {
	...
	public DMS(RemoteWebDriver driver, String strConfig, String strTestingType,  String strUserRole)
			throws InterruptedException {

		CommonLib.getLogger(strConfig).info("Inside login DMS");
...
//		int iCurrentIteration = 1;
		...
		for (int j = 1; j <= iTotalIterations; j++) {
			for (int i = 0; i < businessrules.size(); i++) {
				...
				String DMSUsername = excel.CellValue(configsheet, 5, 1);
				String DMSPassword = excel.CellValue(configsheet, 6, 1);
				
				
				if (Value != "") {
					switch (Header) {

					case "BR_Find_DMSLocation":
						...
						//BusinessRules.searchRisk(driver, strConfig, "25179", true);
						BusinessRules.loginDMS(driver, strConfig, DMSUsername, DMSPassword);
						//BusinessRules.verifyDMSLocation(driver, strConfig);
						break;
