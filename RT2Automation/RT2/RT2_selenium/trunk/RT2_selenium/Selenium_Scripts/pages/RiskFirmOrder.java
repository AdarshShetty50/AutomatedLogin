...
public class RiskFirmOrder {
	...
	public RiskFirmOrder(RemoteWebDriver driver, String strConfig, String strTestingType, String strUserRole)
		...
		for (int j = 1; j <= iTotalIterations; j++) {
			for (int i = 0; i < businessrules.size(); i++) {
				...
				if (Value != "") {
					...
					case "BR_SearchRisk_And_FirmOrder":
						...
						if (rowCountFO > 2) {
							for (int k = 3; k <= rowCountFO; k++) {
								...
								int nullCounter = 0;
								if (!riskFirmOrderData.isEmpty()) {
									for (int m = 0; m < riskFirmOrderData.size(); m++) {
										if (riskFirmOrderData.get(m) == null) {
											nullCounter = nullCounter + 1;
										}
									}

									if (!(nullCounter == riskFirmOrderData.size())) {
										Boolean riskFound = false;
										riskFound = BusinessRules.searchRisk(driver, strConfig,
												riskFirmOrderData.get(0), true);
										if (riskFound) {
							...
							CommonLib.getLogger(strConfig)
									.info("WARNING: No Risk Data supplied for Firm Order in RTTestdata.xlsx file");
							TestReporter.Warning(driver, strConfig, "NTU Risk",
									"No Risk Data supplied for Firm Order in RTTestdata.xlsx file");
