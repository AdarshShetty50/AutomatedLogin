...
public class RiskTrackerLogin {
	...
	public RiskTrackerLogin(RemoteWebDriver driver, String strConfig, String strUserName, String strPassword)
			throws InterruptedException {

		CommonLib.getLogger(strConfig).info("Inside RiskTrackerLogin");
...
//		int iCurrentIteration = 1;
		...
		for (int j = 1; j <= iTotalIterations; j++) {
			for (int i = 0; i < businessrules.size(); i++) {
				...
				if (Value != "") {
					switch (Header) {
					case "BR_Login_RiskTracker":
						
						BusinessRules.loginRiskTracker(driver, strConfig, strUserName, strPassword);
						break;
						
					//Regression Scenario	
					case "BR_Verify_InvalidLogin_For_RiskTracker":
						
						BusinessRules.verify_InvalidLogin_For_RiskTracker(driver, strConfig, strUserName, strPassword);
						break;
